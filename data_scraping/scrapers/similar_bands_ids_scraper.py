...
class SimilarBandsIdsScraper(DataScraper):
    ...
    def _scrap_data(self, response_soup: BeautifulSoup) -> Any:
        similar_bands_ids = map(
            self.__extract_band_id,
            response_soup.find_all('a')
        )

        valid_bands_ids = filter(
            lambda band_id: band_id >= 0,
            similar_bands_ids
        )
